...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    const selectedCity = this.selectedCityService.getCity();
    if (selectedCity) {
      this.getWeatherForCity(selectedCity);
      this.selectedCityService.clearCity();
    } else {
  ...
  isCityFavorite(city: City): boolean {
  ...
  removeFavorite(city: City | CitySearchResult) {
    const ciudad = this.favoriteCities.find(
      (fav) =>
        fav.name === city.name &&
        this.areCoordinatesClose(fav.lat, fav.lon, city.lat, city.lon)
    );
  ...
  getWeatherForCity(city: FavoriteCity) {
  this.loading = true;
  this.weatherService.getWeatherByCoordinates(city.lat, city.lon).subscribe({
    ...
    error: () => {
      this.error = 'No se pudo obtener el clima de la ciudad seleccionada.';
      this.loading = false;
    }
  });
}
